...
# Flask route to handle file upload and SVG conversion
# Flask route to handle file upload and SVG conversion
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        try:
            # Convert the uploaded file to an RGBA PIL Image
            im = Image.open(file.stream).convert('RGBA')
            svg_data = rgba_image_to_svg_contiguous(im)

            # Fixed filename for the SVG output
            svg_filename = 'generated_images/output.svg'
            
            # Save the SVG data to the file, overwriting the existing file if it exists
            with open(svg_filename, 'w') as svg_file:
                svg_file.write(svg_data)
                
            # Optionally, return the path to the saved file or any other success message
            return jsonify({'message': 'SVG file generated successfully.', 'file_path': svg_filename}), 200
